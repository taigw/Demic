...
def binary_assd(s, g, spacing = None):
    ...
    if(len(s.shape) == 2):
        ...
        point_list_s_sample = random.sample(point_list_s, min(500,len(point_list_s)))
        point_list_g_sample = random.sample(point_list_g, min(500,len(point_list_g)))
...
def evaluation(config_file):
    ...
    s_folders = config['segmentation_folder']
    if(type(s_folders) is str):
        s_folders = [s_folders]
    ...
    for metric in metrics:
        for s_folder in s_folders:
            ...
            postfix = patient_names_file.split('/')[-1]
            np.savetxt("{0:}/{1:}_all_{2:}".format(s_folder,  metric, postfix), score_all_data)
            np.savetxt("{0:}/{1:}_mean_{2:}".format(s_folder, metric, postfix), score_mean)
            np.savetxt("{0:}/{1:}_std_{2:}".format(s_folder,  metric, postfix), score_std)
